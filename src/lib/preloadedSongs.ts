import { SongData } from "../services/geminiService";

export const PRELOADED_SONGS: SongData[] = [
  {
    title: "Loser",
    artist: "Beck",
    originalKey: "D",
    suggestedTempo: 85,
    sections: [
      {
        name: "Verse 1",
        lines: [
          "[D]In the time of chimpanzees I was a monkey",
          "Butane in my veins and I'm out to get the junkie",
          "With the plastic eyeballs, spray-paint the vegetables",
          "Dog food stalls with the beefcake pantyhose"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "Soy un per[D]dedor",
          "I'm a loser baby, so why don't you kill me?",
          "(Double-barrel buckshot)"
        ]
      }
    ]
  },
  {
    title: "Champagne Supernova",
    artist: "Oasis",
    originalKey: "A",
    suggestedTempo: 75,
    sections: [
      {
        name: "Verse 1",
        lines: [
          "[Asus2]How many special people change? [G]How many lives are living strange?",
          "[D/F#]Where were you while we were getting [E]high?",
          "[Asus2]Slowly walking down the hall, [G]faster than a cannonball",
          "[D/F#]Where were you while we were getting [E]high?"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "Some [Asus2]day you will [G]find me caught be[D/F#]neath the land[E]slide",
          "In a [Asus2]champagne super[G]nova in the [D/F#]sky  [E]",
          "Some [Asus2]day you will [G]find me caught be[D/F#]neath the land[E]slide",
          "In a [Asus2]champagne super[G]nova, a [D/F#]champagne super[E]nova in the sky"
        ]
      }
    ]
  },
  {
    title: "Wonderwall",
    artist: "Oasis",
    originalKey: "F#m",
    suggestedTempo: 87,
    sections: [
      {
        name: "Verse 1",
        lines: [
          "[Em7]Today is [G]gonna be the day that they're [Dsus4]gonna throw it back to [A7sus4]you",
          "[Em7]By now you [G]should've somehow real[Dsus4]ised what you gotta [A7sus4]do",
          "[Em7]I don't believe that [G]anybody [Dsus4]feels the way I [A7sus4]do about you [Cadd9]now  [Dsus4]  [A7sus4]"
        ]
      },
      {
        name: "Pre-Chorus",
        lines: [
          "And [C]all the roads we [D]have to walk are [Em]winding",
          "And [C]all the lights that [D]lead us there are [G]blinding",
          "There [C]are many [D]things that I would [G]like to [D/F#]say to [Em]you",
          "But I [D]don't know [A]how"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "Because [C]maybe  [Em]  [G]  you're [Em]gonna be the one that [C]saves me  [Em]  [G]",
          "And [Em]after [C]all  [Em]  [G]  you're my [Em]wonder [C]wall  [Em]  [G]  [Em]"
        ]
      }
    ]
  },
  {
    title: "Wish You Were Here",
    artist: "Pink Floyd",
    originalKey: "G",
    suggestedTempo: 60,
    sections: [
      {
        name: "Intro",
        lines: ["[G] [Em7] [G] [Em7] [G] [Em7] [A7sus4] [G]"]
      },
      {
        name: "Verse 1",
        lines: [
          "So, so you think you can [C]tell",
          "Heaven from [D]Hell, blue skies from [Am]pain",
          "Can you tell a green [G]field from a cold steel [D]rail?",
          "A smile from a [C]veil? Do you think you can [Am]tell? [G]"
        ]
      },
      {
        name: "Verse 2",
        lines: [
          "And did they get you to [C]trade",
          "Your heroes for [D]ghosts? Hot ashes for [Am]trees?",
          "Hot air for a [G]cool breeze? Cold comfort for [D]change?",
          "And did you ex[C]change a walk-on part in the [Am]war",
          "For a lead role in a [G]cage?"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "How I wish, how I wish you were [C]here",
          "We're just [D]two lost souls swimming in a fish bowl, [Am]year after year",
          "[G]Running over the same old ground, [D]what have we found?",
          "The same old [C]fears, wish you were [Am]here [G]"
        ]
      },
      {
        name: "Outro Instrumental",
        lines: ["[G] [Em7] [G] [Em7] [G] [Em7] [A7sus4] [G]"]
      }
    ]
  },
  {
    title: "Let It Be",
    artist: "The Beatles",
    originalKey: "C",
    suggestedTempo: 72,
    sections: [
      {
        name: "Verse 1",
        lines: [
          "[C]When I find myself in [G]times of trouble, [Am]Mother Mary [F]comes to me",
          "[C]Speaking words of [G]wisdom, let it [F]be     [C/E]  [Dm]  [C]",
          "[C]And in my hour of [G]darkness, she is [Am]standing right in [F]front of me",
          "[C]Speaking words of [G]wisdom, let it [F]be     [C/E]  [Dm]  [C]"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "Let it [Am]be, let it [G]be, let it [F]be, let it [C]be",
          "[C]Whisper words of [G]wisdom, let it [F]be     [C/E]  [Dm]  [C]"
        ]
      }
    ]
  },
  {
    title: "Yesterday",
    artist: "The Beatles",
    originalKey: "F",
    suggestedTempo: 96,
    sections: [
      {
        name: "Verse 1",
        lines: [
          "[F]Yesterday [Em7]all my [A7]troubles seemed so [Dm]far away  [C]  [Bb]",
          "Now it [C7]looks as though they're [F]here to stay",
          "Oh [Dm7]I be[G7]lieve in [Bb]yester[F]day"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "[Em7]Why [A7]she [Dm]had [C]to [Bb]go I don't [Gm]know [C7]she wouldn't [F]say",
          "[Em7]I [A7]said [Dm]some[C]thing [Bb]wrong now I [Gm]long for [C7]yester[F]day"
        ]
      }
    ]
  },
  {
    title: "Hotel California",
    artist: "The Eagles",
    originalKey: "Bm",
    suggestedTempo: 74,
    sections: [
      {
        name: "Intro",
        lines: ["[Bm] [F#7] [A] [E] [G] [D] [Em] [F#7]"]
      },
      {
        name: "Verse 1",
        lines: [
          "[Bm]On a dark desert highway, [F#7]cool wind in my hair",
          "[A]Warm smell of colitas, [E]rising up through the air",
          "[G]Up ahead in the distance, [D]I saw a shimmering light",
          "[Em]My head grew heavy and my sight grew dim",
          "[F#7]I had to stop for the night"
        ]
      },
      {
        name: "Chorus 1",
        lines: [
          "[G]Welcome to the Hotel Cali[D]fornia",
          "Such a [F#7]lovely place, such a [Bm]lovely face",
          "Plenty of [G]room at the Hotel Cali[D]fornia",
          "Any [Em]time of year, you can [F#7]find it here"
        ]
      },
      {
        name: "Verse 2",
        lines: [
          "[Bm]Her mind is Tiffany-twisted, [F#7]she got the Mercedes bends",
          "[A]She got a lot of pretty, pretty boys [E]she calls friends",
          "[G]How they dance in the courtyard, [D]sweet summer sweat",
          "[Em]Some dance to remember, [F#7]some dance to forget"
        ]
      },
      {
        name: "Verse 3",
        lines: [
          "[Bm]So I called up the Captain, [F#7]\"Please bring me my wine\"",
          "He said, [A]\"We haven't had that spirit here since [E]nineteen sixty nine\"",
          "And [G]still those voices are calling from [D]far away",
          "[Em]Wake you up in the middle of the night",
          "[F#7]Just to hear them say..."
        ]
      },
      {
        name: "Chorus 2",
        lines: [
          "[G]Welcome to the Hotel Cali[D]fornia",
          "Such a [F#7]lovely place (Such a lovely place)",
          "[Bm]Such a lovely face",
          "They [G]livin' it up at the Hotel Cali[D]fornia",
          "What a [Em]nice surprise (What a nice surprise)",
          "[F#7]Bring your alibis"
        ]
      },
      {
        name: "Verse 4",
        lines: [
          "[Bm]Last thing I remember, I was [F#7]running for the door",
          "[A]I had to find the passage back to the [E]place I was before",
          "[G]\"Relax,\" said the night man, \"We are [D]programmed to receive",
          "[Em]You can check out any time you like, [F#7]but you can never leave!\""
        ]
      },
      {
        name: "Outro Solo",
        lines: [
          "[Bm] [F#7] [A] [E] [G] [D] [Em] [F#7]",
          "(Repeat progression for massive guitar solo to fadeout)"
        ]
      }
    ]
  },
  {
    title: "Sweet Child O' Mine",
    artist: "Guns N' Roses",
    originalKey: "Db",
    suggestedTempo: 125,
    sections: [
      {
        name: "Intro",
        lines: ["[Db] [Gb] [Db] [Ab] [Gb] [Db]"]
      },
      {
        name: "Verse 1",
        lines: [
          "[Db]She's got a smile that it seems to me",
          "Re[Gb]minds me of childhood memories",
          "Where [Db]everything was as fresh as the bright blue [Ab]sky [Gb] [Db]",
          "[Db]Now and then when I see her face",
          "She [Gb]takes me away to that special place",
          "And if I [Db]stared too long, I'd probably break down and [Ab]cry [Gb] [Db]"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "[Ab]Whoa, [Gb]oh, oh, [Db]Sweet child o' mine",
          "[Ab]Whoa, [Gb]oh, oh, oh, [Db]Sweet love of mine"
        ]
      },
      {
        name: "Verse 2",
        lines: [
          "[Db]She's got eyes of the bluest skies",
          "As [Gb]if they thought of rain",
          "I [Db]hate to look into those eyes and [Ab]see an [Gb]ounce of [Db]pain",
          "Her [Db]hair reminds me of a warm safe place",
          "Where [Gb]as a child I'd hide",
          "And [Db]pray for the thunder and the rain to [Ab]quiet[Gb]ly pass me [Db]by"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "[Ab]Whoa, [Gb]oh, oh, [Db]Sweet child o' mine",
          "[Ab]Whoa, [Gb]oh, oh, oh, [Db]Sweet love of mine"
        ]
      },
      {
        name: "Bridge",
        lines: [
          "[D#m]Where do we go? [B]Where do we go now? [Gb]Where do we go? [Db]",
          "[D#m]Where do we go? [B]Where do we go now? [Gb]Where do we go? [Db]",
          "[D#m]Where do we go? [B]Where do we go now? [Gb]Where do we go? [Db]",
          "[D#m]Ay, [B]ay, ay, ay, ay, ay, ay, [Gb]ay [Db]",
          "[D#m]Where do we go now? [B] [Gb] [Db]",
          "[D#m]Where do we goooooooo? [B] [Gb] [Db] [D#m]"
        ]
      }
    ]
  },
  {
    title: "Smells Like Teen Spirit",
    artist: "Nirvana",
    originalKey: "Fm",
    suggestedTempo: 117,
    sections: [
      {
        name: "Intro",
        lines: ["[F5] [Bb5] [Ab5] [Db5]"]
      },
      {
        name: "Verse 1",
        lines: [
          "[F5]Load up on [Bb5]guns, [Ab5]bring your [Db5]friends",
          "[F5]It's fun to [Bb5]lose and [Ab5]to pre[Db5]tend",
          "[F5]She's over-[Bb5]board and [Ab5]self-as[Db5]sured",
          "[F5]Oh no, I [Bb5]know a [Ab5]dirty [Db5]word"
        ]
      },
      {
        name: "Chorus",
        lines: [
          "With the [F5]lights out, [Bb5]it's less [Ab5]dangerous [Db5]",
          "Here we [F5]are now, [Bb5]enter[Ab5]tain us [Db5]"
        ]
      }
    ]
  },
];
